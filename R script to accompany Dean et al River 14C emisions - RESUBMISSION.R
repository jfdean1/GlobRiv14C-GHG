...
## 1A global map of sample locations ##
...
## Supplementary Fig. S1 - F14Catm histogram ##
...
title({'F^{14}C_{atm}', 'Small catchments (â‰¤10 kmÂ²)'});
xlabel('IncMSE');
hold on;
...
% Configure axes and add significant markers
set(gca, 'YTick', 1:length(AIncMSE), 'YTickLabel', flip(ACDname), 'FontSize', 13);
title({'F^{14}C_{atm}', 'Large catchments (>10 kmÂ²)'});
xlabel('IncMSE');
xlim([0,32])
hold on;
